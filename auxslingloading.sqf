...
AuxSling_light_config = AuxSling_light_config + []; //lift = 500 e.g. Littlebird
AuxSling_medium_config = AuxSling_light_config + AuxSling_medium_config + []; //lift = 4000 e.g GhostHawk
...
AuxSling_fnc_InList = {
...

	{
		...
		if (_vehweight > 2000 && !_found) then {
			{
...
AuxSling_fnc_Conditions1 = {
	...
	if !(isNull _obj) then {
		_vehname = format ["<t color='#449121'>Kargo Halatı - %1</t>",getText (configfile >> "CfgVehicles" >> typeOf _obj >> "displayName")];
		_unit setUserActionText [(_unit getVariable "AuxSling_Load_Action"),_vehname];
	} else {
		_unit setUserActionText [(_unit getVariable "AuxSling_Load_Action"),"<t color='#449121'>Uyumsuz Araç</t>"];
	};
...
AuxSling_fnc_Conditions2 = {
	...
	_vehname = format ["<t color='#449121'>Halatı Çöz - %1</t>",getText (configfile >> "CfgVehicles" >> typeOf (_veh getVariable "AuxSling_AttachedObject") >> "displayName")];
...
AuxSling_fnc_AddAction = {
	...
	if (isNil {_unit getVariable "AuxSling_Added"}) then {
		AuxSling_Load_Action = _unit addAction ["<t color='#449121'>Kargo Halatı</t>",{[(_this select 0)] spawn AuxSling_fnc_DoAttaching},"",-99,true,true,"",'[_this,_target] call AuxSling_fnc_Conditions1'];
		_unit setVariable ["AuxSling_Load_Action",AuxSling_Load_Action,true];
		AuxSling_unLoad_Action = _unit addAction ["<t color='#449121'>Halatı Çöz</t>",{[(_this select 0)] spawn AuxSling_fnc_DoDetaching},"",-99,false,true,"",'[_this,_target] call AuxSling_fnc_Conditions2'];
